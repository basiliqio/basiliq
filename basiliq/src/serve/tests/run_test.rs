...
macro_rules! run_test_request {
    ($name:ident, $method:expr, $query:literal, $expected_status:literal) => {
        #[basiliq_test(run_migrations, init_values)]
        async fn $name(pool: sqlx::PgPool) {
            let state = prepare_basiliq_test(pool).await;

            let res = crate::serve::tests::run_request(state, $method, $query, $expected_status, None).await;
			insta::assert_json_snapshot!(res);
        }
    };

	($name:ident, $method:expr, $query:literal, $expected_status:literal, $body:expr) => {
		#[basiliq_test(run_migrations, init_values)]
		async fn $name(pool: sqlx::PgPool) {
			let state = prepare_basiliq_test(pool).await;

			let res = crate::serve::tests::run_request(state, $method, $query, $expected_status, Some($body)).await;
			insta::assert_json_snapshot!(res,
			{
